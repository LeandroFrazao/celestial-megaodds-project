...
const inputPath =
  getArg("input") || path.join(__dirname, "..", "data", "mega_sena_astro.json");
...
const targetHour = Number(getArg("hour", "21"));
const targetMinute = Number(getArg("minute", "0"));
const ticketsCount = Number(getArg("tickets", String(fileConfig.ticketsPerDraw || 10)));
const latitude = Number(getArg("lat", "-23.564"));
const longitude = Number(getArg("lon", "-46.651"));

if (!targetDate || !/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) {
  console.error("Usage: node scripts/predict_next.js --date YYYY-MM-DD [--hour 0-23] [--minute 0-59] [--tickets N] [--lat -23.564] [--lon -46.651]");
  process.exit(1);
}

const stamp = `${targetDate}_${String(targetHour).padStart(2, "0")}${String(targetMinute).padStart(2, "0")}`;
const outputPath =
  getArg("out") || path.join(__dirname, "..", "data", `predictions_${stamp}.json`);
const outputCsv =
  getArg("csv") || path.join(__dirname, "..", "data", `predictions_${stamp}.csv`);
...
const minHistory = Number(process.env.MIN_HISTORY || fileConfig.minHistory || 50);
const seed = Number(process.env.SEED || fileConfig.seed || 42);
const coldWindow = Number(process.env.COLD_WINDOW || fileConfig.coldWindow || 25);
...
function numbersFromDraw(draw) {
  return [draw.bola1, draw.bola2, draw.bola3, draw.bola4, draw.bola5, draw.bola6].map(Number);
}
...
function componentWeights(window, targetAstro) {
  ...
  for (let i = 0; i < window.length; i += 1) {
    ...
    if (simScore > 0) {
      for (const n of nums) astro[n] += simScore;
    }
  }
...
function buildTargetAstro(dateStr, hour, minute, lat, lon) {
  ...
  for (const key of Object.keys(horoscope.CelestialBodies)) {
    ...
    bodies[key] = {
      ...
      chart: {
        ecliptic_deg: horoscope.CelestialBodies[key].ChartPosition?.Ecliptic?.DecimalDegrees ?? null,
      },
    };
  }
  ...
  const phaseAngle = moonEcl != null && sunEcl != null ? ((moonEcl - sunEcl) % 360 + 360) % 360 : null;

  const moonPhaseFromAngle = (angle) => {
  ...
  const sumDigits = (str) => String(str).replace(/\D/g, "").split("").reduce((sum, d) => sum + Number(d), 0);
  ...
  const dateDigitSum = sumDigits(dateStr);
  ...
  return {
    ...
    lunar_phase: {
      ...
      illumination: phaseAngle != null ? (1 - Math.cos((Math.PI / 180) * phaseAngle)) / 2 : null,
...
const targetAstro = buildTargetAstro(targetDate, targetHour, targetMinute, latitude, longitude);
...
for (let n = 1; n <= 60; n += 1) {
  mixed[n] = comps.freq[n] * coeff.freq + comps.recency[n] * coeff.recency + comps.astro[n] * coeff.astro;
}

const uniform = normalizeWeights(comps.base);
const mixedNorm = normalizeWeights(mixed);
const blended = mixedNorm.map((w, idx) => w * (1 - explore) + uniform[idx] * explore);
...
const csvRows = [["ticket", "n1", "n2", "n3", "n4", "n5", "n6", "entropy"]];
